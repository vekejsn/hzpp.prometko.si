            ...
            vehicles.forEach(async vehicle => {
                ...
                marker.getElement().addEventListener('click', async () => {
                    while (true) {
                        ...
                        const $scheduleTable = zs_makeScheduleTable(marker);
                        const compositionText = marker.data.train_cache.composition;
                        showSidebar({
                            content: /*html*/`
                            <div class="sidebar-inner" id="sidebar-zs-${marker.data.train_data.train_number}">
...
function zs_makeScheduleTable(marker) {
    ...
    for (let i = 0; i < marker.data.train_data.train_times.length; i++) {
        ...
        let arrival_time = luxon.DateTime.fromFormat(stopTime.arrival_time, 'H:m:s').setZone('Europe/Ljubljana');
        ...
        let departure_time = luxon.DateTime.fromFormat(stopTime.departure_time, 'H:m:s').setZone('Europe/Ljubljana');
